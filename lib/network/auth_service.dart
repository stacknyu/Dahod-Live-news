...
import 'package:news_flutter/model/base_response.dart';
import 'package:news_flutter/network/network_utils.dart';
...
Future<User> LogInWithGoogle() async {
  ...
  if (googleSignInAccount != null) {
    ...
    return await login(req, isSocialLogin: true).then((value) {
      ...
      OneSignal.login(currentUser.email.validate());
      return currentUser;
    }).catchError((e) {
      appStore.setLoading(false);
      toast(e.toString());
...
Future<void> signInWithOTP(BuildContext context, String phoneNumber, {Function()? callBack}) async {
  await _auth.verifyPhoneNumber(
    ...
    verificationFailed: (FirebaseAuthException e) {
      if (e.code == 'invalid-phone-number') {
        toast(language.lblInvalidPhoneNumber);
...
Future<BaseResponse> deleteAccount() async {
  return BaseResponse.fromJson(await handleResponse(await deleteRequest('iqonic-api/api/v1/user/delete-account', requireToken: true, request: {})));
}
